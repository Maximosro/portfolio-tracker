...
class PriceHistoryPurgeServiceTest {
    ...
    /**
     * Registros de hace 10 días (zona horaria): con 6 registros en la misma hora,
     * debe eliminar 5 y mantener solo el último de cada hora.
     */
    @Test
    ...
    /**
     * Registros de hace 45 días (zona diaria): con 3 registros en el mismo día,
    ...
    void purge_singleRecordPerBucket_noDeletes() {
        ...
        List<PriceHistory> records = List.of(
                PriceHistory.builder().id(1L).ticker(TICKER).timestamp(base).rawPrice(100.0).currency("USD").priceEur(95.0).build(),
                PriceHistory.builder().id(2L).ticker(TICKER).timestamp(base.plus(1, ChronoUnit.HOURS)).rawPrice(101.0).currency("USD").priceEur(96.0).build(),
                PriceHistory.builder().id(3L).ticker(TICKER).timestamp(base.plus(2, ChronoUnit.HOURS)).rawPrice(102.0).currency("USD").priceEur(97.0).build()
        );
    ...
    void purge_multipleTickersCompactedIndependently() {
        ...
        List<PriceHistory> aaplRecords = List.of(
                PriceHistory.builder().id(1L).ticker("AAPL").timestamp(base).rawPrice(150.0).currency("USD").priceEur(140.0).build(),
                PriceHistory.builder().id(2L).ticker("AAPL").timestamp(base.plus(5, ChronoUnit.MINUTES)).rawPrice(151.0).currency("USD").priceEur(141.0).build()
        );
        List<PriceHistory> msftRecords = List.of(
                PriceHistory.builder().id(3L).ticker("MSFT").timestamp(base).rawPrice(300.0).currency("USD").priceEur(280.0).build(),
                PriceHistory.builder().id(4L).ticker("MSFT").timestamp(base.plus(5, ChronoUnit.MINUTES)).rawPrice(301.0).currency("USD").priceEur(281.0).build(),
                PriceHistory.builder().id(5L).ticker("MSFT").timestamp(base.plus(10, ChronoUnit.MINUTES)).rawPrice(302.0).currency("USD").priceEur(282.0).build()
        );
    ...
    void purge_noTickers_returnsZero() {
        ...
        assertEquals(0, removed);
        verify(priceHistoryRepository, never()).findByTickerAndTimestampBetweenOrderByTimestampAsc(any(), any(), any());
    }
