  ...
  public YahooQuoteExtended fetchQuoteExtended(String yahooTicker) {
    try {
      ...
      if (closes != null && closes.length > 0) {
        ...
        if (closes[weekIdx] > 0) {
          changePctWeek =
              Math.round(((currentPrice - closes[weekIdx]) / closes[weekIdx]) * 10000.0) / 100.0;
        }
      ...
      Long volume =
          meta.has("regularMarketVolume") ? meta.path("regularMarketVolume").asLong(0) : null;
      ...
      return new YahooQuoteExtended(currentPrice, currency, changePctDay, changePctWeek,
          changePctMonth, volume, avgVolume);

    } catch (Exception e) {
      log.error("Error obteniendo cotización extendida de {}: {}", yahooTicker, e.getMessage());
      throw new RuntimeException("Error fetching extended " + yahooTicker + ": " + e.getMessage(),
          e);
    }
  }
}
