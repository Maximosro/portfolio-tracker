...
public class WatchlistAlertService {
  ...
  public WatchlistAlert update(Long id, WatchlistAlert updated) {
    ...
    if (updated.getAlertType() != null) {
      boolean changed = !existing.getAlertType().equals(updated.getAlertType())
          || (updated.getThreshold() != null && updated.getThreshold() > 0
          && !existing.getThreshold().equals(updated.getThreshold()));
      if (changed) {
  ...
  /**
   * Evalúa todas las alertas habilitadas y devuelve las que se disparan. Marca las disparadas con
   * triggered=true y lastTriggeredAt. Resetea triggered si la condición ya no se cumple y el
   * cooldown ha pasado.
   */
  @Transactional
  ...
  private boolean evaluateCondition(WatchlistAlert alert, WatchlistItem item) {
    return switch (alert.getAlertType()) {
      ...
      case VOLUME_ABOVE -> {
        if (item.getVolume() == null || item.getAvgVolume() == null || item.getAvgVolume() == 0) {
          yield false;
        }
  ...
  private String buildMessage(WatchlistAlert alert, WatchlistItem item) {
    return switch (alert.getAlertType()) {
      case PRICE_ABOVE ->
          String.format("Precio (%.2f €) ha superado %.2f €", item.getCurrentPrice(),
              alert.getThreshold());
      case PRICE_BELOW ->
          String.format("Precio (%.2f €) ha caído bajo %.2f €", item.getCurrentPrice(),
              alert.getThreshold());
      case VOLUME_ABOVE -> {
  ...
  private static String fmtVol(Long v) {
    ...
    if (v >= 1_000) {
      return String.format("%dK", v / 1_000);
    }
    return v.toString();
  }
