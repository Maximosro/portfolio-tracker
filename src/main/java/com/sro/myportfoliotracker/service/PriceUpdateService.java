...
public class PriceUpdateService {
  ...
  public void scheduledUpdate() {
    ...
    log.info("⏰ Actualización programada de precios (ciclo {}, {})", cycle,
        extended ? "extendido" : "ligero");
    activityLog.info("PRICE",
        "Actualización programada de precios iniciada" + (extended ? " (extendido)" : ""), null,
        "⏰");
  ...
  /**
   * Si hay posiciones que fallaron y están en la ventana post-cierre, reintentar hasta
   * POST_CLOSE_MAX_RETRIES veces para capturar el precio de cierre.
   */
  private void retryPostCloseFailures(PriceUpdateResult result, boolean extended) {
    if (result.failedTickers() == null || result.failedTickers().isEmpty()) {
      return;
    }
    ...
    List<String> postCloseFailures = result.failedTickers().stream()
        .filter(ticker -> {
          return positionRepository.findById(ticker.replaceAll("\\s*\\(.*\\)", "").trim())
              .map(p -> p.getYahooTicker() != null && marketScheduleService.isInPostCloseGrace(
                  p.getYahooTicker()))
              .orElse(false);
        })
        .toList();

    if (postCloseFailures.isEmpty()) {
      return;
    }

    log.info(
        "🔄 {} posiciones en post-cierre fallaron, reintentando (máx {} intentos, 1 por minuto)",
        postCloseFailures.size(), POST_CLOSE_MAX_RETRIES);
    activityLog.info("PRICE",
        postCloseFailures.size() + " posiciones en post-cierre fallaron, reintentando...", null,
        "🔄");
    ...
    for (int attempt = 1; attempt <= POST_CLOSE_MAX_RETRIES; attempt++) {
      ...
      for (String tickerRaw : postCloseFailures) {
        ...
        if (position == null || position.getYahooTicker() == null) {
          continue;
        }
        ...
        try {
          ...
          double priceEur =
              Math.round(exchangeRateService.convertToEur(rawPrice, currency) * 10000.0) / 10000.0;
          position.setCurrentPrice(priceEur);
          position.setLastPriceUpdate(now);
          positionRepository.save(position);
          priceHistoryRepository.save(PriceHistory.builder()
              .ticker(ticker).timestamp(now).rawPrice(rawPrice).currency(currency)
              .priceEur(priceEur).build());
          log.info("✓ Reintento {}/{}: {} → {} EUR", attempt, POST_CLOSE_MAX_RETRIES, ticker,
              priceEur);
          activityLog.success("PRICE",
              "Reintento post-cierre OK: " + ticker + " → " + priceEur + " EUR", ticker, "🔄");
          Thread.sleep(500);
        } catch (InterruptedException e) {
          Thread.currentThread().interrupt();
          return;
        } catch (Exception e) {
          log.warn("✗ Reintento {}/{}: {} — {}", attempt, POST_CLOSE_MAX_RETRIES, ticker,
              e.getMessage());
      ...
      if (stillFailing.isEmpty()) {
        ...
        activityLog.success("PRICE",
            "Precios post-cierre recuperados tras " + attempt + " reintento(s)", null, "✅");
        return;
      }
    ...
    activityLog.error("PRICE",
        postCloseFailures.size() + " posiciones post-cierre sin precio real tras reintentos: "
            + postCloseFailures, null, "⚠");
  }

  /**
   * A medianoche, guardar el precio actual como precio de cierre del día (previousClose). Esto
   * permite calcular la variación diaria al día siguiente.
   */
  ...
  /**
   * Actualiza precios de todas las posiciones con yahooTicker definido. Guarda cada precio obtenido
   * en el histórico. NO es @Transactional a propósito: cada posición se guarda independientemente
   * para que un fallo en una no impida guardar el resto.
   *
   * @param extended si true, usa fetchQuoteExtended (range=1mo, incluye volumen). Si false, usa
   *                 fetchQuote (range=1d, más ligero).
   */
  ...
  public PriceUpdateResult updateAllPrices(boolean extended, boolean force) {
    ...
    for (Position position : positions) {
      ...
      if (!force && !marketScheduleService.isMarketOpen(yahoo)) {
        log.debug("⏸ {} ({}) mercado cerrado — manteniendo último precio", position.getTicker(),
            yahoo);
        skippedMarketClosed++;
      ...
      try {
        ...
        log.info("✓ {} ({}) → {} {} → {} EUR", position.getTicker(), yahoo, rawPrice, currency,
            priceEur);
        activityLog.success("PRICE",
            position.getTicker() + " → " + priceEur + " EUR (vía " + yahoo + ")",
            position.getTicker(), "📈");
        ...
        activityLog.error("PRICE",
            "Error actualizando " + position.getTicker() + ": " + e.getMessage(),
            position.getTicker(), "❌");
        ...
        if (position.getCurrentPrice() != null) {
          priceHistoryRepository.save(PriceHistory.builder()
              ...
              .rawPrice(position.getCurrentPrice()).currency("EUR")
              .priceEur(position.getCurrentPrice()).build());
          log.info("↻ {} — histórico con precio existente ({} EUR)", position.getTicker(),
    ...
    log.info("Actualización completada: {} OK, {} errores, {} omitidos (mercado cerrado)", updated,
        failed.size(), skippedMarketClosed);
    activityLog.success("PRICE",
        "Actualización completada: " + updated + " OK, " + failed.size() + " errores, "
            + skippedMarketClosed + " omitidos (mercado cerrado)", null, "✅");

    try {
      telegramService.checkAndNotifyAlerts();
    } catch (Exception e) {
      log.debug("Error revisando alertas Telegram tras actualización de precios: {}",
          e.getMessage());
    }
