...
import com.sro.myportfoliotracker.dto.simulator.MortgageResult.SensitivityRow;
import com.sro.myportfoliotracker.dto.simulator.MortgageResult.StrategyResult;
import com.sro.myportfoliotracker.dto.simulator.MortgageResult.YearSnapshot;
...
public class MortgageSimulatorService {
  ...
  public MortgageResult simulate(MortgageRequest req) {
    ...
    double[] monthlyPayments = buildMonthlyPaymentSchedule(req, principal, monthlyPayment,
        remainingYears);
    ...
    StrategyResult amortizeResult = simulateAmortize(principal, monthlyRates, monthlyPayments,
        extra, remainingYears, currentPortfolio, currentInvested, investReturn, taxRate);

    // ─── 3. Estrategia INVERTIR ───
    StrategyResult investResult = simulateInvest(principal, monthlyRates, monthlyPayments, extra,
        remainingYears, currentPortfolio, currentInvested, investReturn, taxRate);
    ...
    // ─── 4. Evolución anual ───
    List<YearSnapshot> evolution = buildEvolution(principal, monthlyRates, monthlyPayments, extra,
        remainingYears, currentPortfolio, investReturn);

    // ─── 5. Sensibilidad ───
    List<SensitivityRow> sensitivity = buildSensitivity(principal, annualRate, extra,
        remainingYears, currentPortfolio, currentInvested, taxRate);
  ...
  /**
   * Builds an array of monthly interest rates (as decimal) for each month of the mortgage. Supports
   * mixed mortgages with multiple rate periods.
   */
  ...
  private StrategyResult simulateAmortize(double principal, double[] monthlyRates,
  ...
  private StrategyResult simulateInvest(double principal, double[] monthlyRates,
      double[] monthlyPayments,
      double extra, int years, double portfolioStart, double investedStart,
      double investReturn, double taxRate) {
    double debt = principal;
  ...
  private List<YearSnapshot> buildEvolution(double principal, double[] monthlyRates,
      double[] monthlyPayments,
  ...
  private List<SensitivityRow> buildSensitivity(double principal, double annualRate,
    ...
    for (double r : returns) {
      for (double rate : rates) {
        ...
        for (int i = 0; i < totalMonths; i++) {
          fixedRates[i] = monthlyRate;
          fixedPayments[i] = mp;
        }

        StrategyResult amort = simulateAmortize(principal, fixedRates, fixedPayments, extra, years,
            portfolioStart, investedStart, r, taxRate);
        StrategyResult invest = simulateInvest(principal, fixedRates, fixedPayments, extra, years,
            portfolioStart, investedStart, r, taxRate);

        String winner =
            invest.getNetWealthEnd() > amort.getNetWealthEnd() ? "INVERTIR" : "AMORTIZAR";
  ...
  /**
   * Builds monthly payment schedule. For mixed mortgages, recalculates the payment at each period
   * transition based on remaining principal and remaining term.
   */
  private double[] buildMonthlyPaymentSchedule(MortgageRequest req, double principal,
      double initialPayment, int remainingYears) {
    int totalMonths = remainingYears * 12;
    ...
    for (int p = 0; p < req.getRatePeriods().size() && monthIdx < totalMonths; p++) {
      ...
      double periodPayment = calculateMonthlyPayment(debt, annualRate,
          (totalMonths - monthsElapsed + 11) / 12);
    ...
    if (monthIdx < totalMonths && monthIdx > 0) {
      for (; monthIdx < totalMonths; monthIdx++) {
        payments[monthIdx] = payments[monthIdx - 1];
      }
    }
  ...
  private double calculateMonthlyPayment(double principal, double annualRate, int years) {
    ...
    double monthlyRate = annualRate / 100.0 / 12.0;
    int n = years * 12;
    return principal * monthlyRate * Math.pow(1 + monthlyRate, n) / (Math.pow(1 + monthlyRate, n)
        - 1);
  }
