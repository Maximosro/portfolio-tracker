...
public class ProjectionSimulatorService {
  ...
  public ProjectionResult simulate(ProjectionRequest request) {
    ...
    List<YearProjection> baseProjection = project(currentValue, currentInvested,
        monthlyContribution, expectedReturn, inflationPct, years);
    List<YearProjection> pessimistic = null;
    List<YearProjection> optimistic = null;

    if (Boolean.TRUE.equals(request.getShowScenarios())) {
      pessimistic = project(currentValue, currentInvested, monthlyContribution,
          Math.max(expectedReturn - 3, 0), inflationPct, years);
      optimistic = project(currentValue, currentInvested, monthlyContribution, expectedReturn + 3,
          inflationPct, years);
    }
    ...
    Summary summary = Summary.builder()
        ...
        .finalValuePessimistic(
            pessimistic != null ? pessimistic.get(pessimistic.size() - 1).getPortfolioValue() : 0)
        .finalValueOptimistic(
            optimistic != null ? optimistic.get(optimistic.size() - 1).getPortfolioValue() : 0)
  ...
  private List<YearProjection> project(double startValue, double startInvested,
      double monthlyContribution, double annualReturnPct, double inflationPct, int years) {
