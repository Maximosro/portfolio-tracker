 ...
 * <p>
 * Telegram es COMPLETAMENTE OPCIONAL. Si no se configura, el servicio permanece inactivo y no
 * produce ningún error ni warning.
 * <p>
 * La configuración se persiste en la tabla app_settings de la BD, por lo que sobrevive a reinicios
 * de la aplicación. También acepta configuración inicial desde application.yaml.
 */
@Service
@Slf4j
public class TelegramService {
  ...
  public TelegramService(RestClient restClient, AlertService alertService,
  ...
  /**
   * Al iniciar la app, carga la configuración persistida en BD (si existe). Los valores de BD
   * tienen prioridad sobre application.yaml. Si Telegram no está configurado, simplemente no hace
   * nada (sin errores ni warnings).
   */
  @PostConstruct
  public void loadPersistedConfig() {
    try {
      ...
      if (isEnabled()) {
        ...
        log.info(
            "ℹ️ Telegram: no configurado — las notificaciones por Telegram están desactivadas (esto es opcional)");
      }
    } catch (Exception e) {
      // No loguear como warning/error: Telegram es opcional y puede no haber tabla aún
      log.debug(
          "Telegram: no se pudo acceder a la configuración en BD (tabla puede no existir aún): {}",
          e.getMessage());
      this.enabled = false;
    }
  }
  ...
  private void persistConfig() {
    ...
    settingRepository.save(
        AppSetting.builder().key(SETTING_ENABLED).value(String.valueOf(enabled)).build());
  }

  /**
   * Envía un mensaje de texto a través del bot de Telegram. Si Telegram no está
   * configurado/habilitado, retorna false silenciosamente.
   */
  ...
  /**
   * Revisa las alertas y envía notificaciones solo para alertas NUEVAS. Usa
   * PositionAlert.notifiedTelegram para evitar reenvíos (persistente). Se invoca automáticamente
   * tras cada actualización de precios. Si Telegram no está configurado, no hace nada (sin
   * errores).
   */
  public void checkAndNotifyAlerts() {
    ...
    try {
      ...
      for (AlertDto alert : currentAlerts) {
        ...
        if (pa == null || Boolean.TRUE.equals(pa.getNotifiedTelegram())) {
          continue;
        }
        ...
        if (sendMessage(message)) {
          ...
          activityLog.info("TELEGRAM",
              "Alerta notificada: " + alert.getTicker() + " — " + alert.getType(),
              alert.getTicker(), "🔔");
        }
      }
  ...
  public boolean sendWatchlistAlert(WatchlistItem item, WatchlistAlert alert, String message) {
    ...
    if (sent) {
      activityLog.info("TELEGRAM", "Alerta watchlist: " + item.getTicker() + " — " + typeLabel,
          item.getTicker(), "👁");
    }
    return sent;
  }
  ...
  /**
   * Envía un informe de cierre de mercado por Telegram. Se ejecuta a las 18:00 L-V (cierre de
   * mercado europeo). Incluye: valor total, P&L del día, top gainers/losers, TIR y alertas.
   */
  ...
  public void sendDailySummary() {
    ...
    try {
      ...
      for (int i = 0; i < positions.size(); i++) {
        ...
        double cost =
            p.getShares() * (p.getAvgPrice() != null ? p.getAvgPrice() : p.getCurrentPrice());
        totalValue += value;
        totalCost += cost;
        ...
        if (p.getPreviousClose() != null && p.getPreviousClose() > 0) {
          double changePct =
              ((p.getCurrentPrice() - p.getPreviousClose()) / p.getPreviousClose()) * 100;
      ...
      if (dayChanges.size() >= 2) {
        ...
        for (int i = dayChanges.size() - 1; i >= Math.max(0, dayChanges.size() - topN); i--) {
          Position p = positions.get((int) dayChanges.get(i)[1]);
          double pct = dayChanges.get(i)[0];
          if (pct >= 0 && i < dayChanges.size() - 1) {
            break; // todas positivas, no listar
          }
          sb.append(String.format("  %s  %s%%\n", p.getTicker(), fmtSign(pct)));
        }
      }
      ...
      if (alerts != null && !alerts.isEmpty()) {
        List<AlertDto> dangerAlerts = alerts.stream().filter(a -> "DANGER".equals(a.getSeverity()))
            .toList();
        List<AlertDto> warningAlerts = alerts.stream()
            .filter(a -> "WARNING".equals(a.getSeverity())).toList();

        if (!dangerAlerts.isEmpty()) {
          sb.append(String.format("\n🚨 <b>%d alerta(s) crítica(s):</b>\n", dangerAlerts.size()));
          dangerAlerts.forEach(
              a -> sb.append(String.format("  🔴 <b>%s</b>: %s\n", a.getTicker(), a.getMessage())));
        }
