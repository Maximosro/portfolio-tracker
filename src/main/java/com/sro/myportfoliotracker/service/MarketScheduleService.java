...
public class MarketScheduleService {
  ...
  /**
   * Minutos de gracia tras el cierre para hacer una última actualización y capturar el precio de
   * cierre real.
   */
  private static final int POST_CLOSE_GRACE_MINUTES = 10;

  /**
   * Determina si el mercado del ticker dado está abierto en este momento, o si está dentro de la
   * ventana de gracia post-cierre (10 min). - Extrae el sufijo del ticker (ej: ".DE" de "SXR8.DE")
   * - Busca la configuración en BD - Si no hay registro o no está habilitado → siempre abierto
   * (true) - Si es fin de semana en la zona horaria del mercado → cerrado - Si la hora local está
   * dentro del horario o en la ventana de gracia → abierto
   */
  ...
  /**
   * Indica si un ticker está actualmente en la ventana post-cierre (entre closeTime y closeTime +
   * gracia).
   */
  public boolean isInPostCloseGrace(String yahooTicker) {
    ...
    try {
      ...
      LocalTime closeTime = LocalTime.parse(schedule.getCloseTime(), TIME_FMT);
      LocalTime currentTime = now.toLocalTime();
      return currentTime.isAfter(closeTime) && !currentTime.isAfter(
          closeTime.plusMinutes(POST_CLOSE_GRACE_MINUTES));
    } catch (Exception e) {
      return false;
    }
  }
  ...
  /**
   * Extrae el sufijo del ticker Yahoo. "SXR8.DE" → ".DE", "AAPL" → null, "VUAA.L" → ".L"
   */
