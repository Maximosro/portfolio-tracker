...
public class NewsService {
    ...
    /**
     * Busca noticias recientes de Google News RSS para cada posición de la cartera.
     * Cada noticia incluye el ticker relacionado y se ordenan por fecha descendente.
     */
    ...
    public List<NewsItem> fetchNews() {
        ...
        for (Position p : positions) {
            ...
            for (String query : queries) {
                if (unique.size() >= NEWS_PER_ETF) break;
        ...
        List<NewsItem> result = newsByTicker.values().stream()
                .flatMap(List::stream)
                .sorted((a, b) -> {
                    if (a.dateIso() == null || a.dateIso().isEmpty()) return 1;
                    if (b.dateIso() == null || b.dateIso().isEmpty()) return -1;
    ...
    /**
     * Construye varias queries para un ETF, de más específica a más genérica.
     * Todas las queries incluyen contexto financiero para evitar resultados irrelevantes.
     */
        ...
        if (name != null && !name.isBlank()) {
            String keywords = Arrays.stream(name.split("\\s+"))
                    ...
                    .filter(w -> !Set.of("UCITS", "Dist", "Acc", "EUR", "USD", "GBP", "ETF", "Fund", "Index",
                                    "iShares", "Xtrackers", "Amundi", "SPDR", "Vanguard", "Invesco", "VanEck", "Core", "Global")
                            .contains(w))
                    .limit(3)
    ...
    /**
     * Comprueba si el título de una noticia es financieramente relevante.
     * Busca la presencia de al menos una palabra clave financiera.
     */
    private boolean isFinanciallyRelevant(String title, String ticker, String sector) {
        ...
        if (lower.contains(ticker.toLowerCase())) return true;
