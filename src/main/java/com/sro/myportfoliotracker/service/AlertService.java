...
public class AlertService {
  ...
  /**
   * Devuelve las alertas de hoy combinando los registros persistidos con las condiciones actuales.
   * Cada par (ticker, alertType) solo genera una alerta por día natural.
   */
  ...
  /**
   * @deprecated Usar {@link #getTodayAlerts()} que añade cooldown diario. Mantenido por
   * compatibilidad con ExportService.
   */
   ...
   */
  private List<AlertDto> evaluateConditions(Map<String, Position> posMap,
      List<PositionDetail> details) {
    ...
    for (PositionDetail detail : details) {
      ...
      if (detail.getStopLoss() != null && detail.getStopLoss() > 0) {
        ...
        if (price <= detail.getStopLoss()) {
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("STOP_LOSS")
              .severity("DANGER").message(
                  "Precio (" + fmt(price) + " €) ha alcanzado o roto el Stop-Loss (" + fmt(
                      detail.getStopLoss()) + " €)").currentPrice(price)
              .limitPrice(detail.getStopLoss()).distancePct(round2(dist)).build());
        } else if (dist < 5) {
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("STOP_LOSS")
              .severity("WARNING").message(
                  "Precio a solo " + fmt(dist) + "% del Stop-Loss (" + fmt(detail.getStopLoss())
                      + " €)").currentPrice(price).limitPrice(detail.getStopLoss())
              .distancePct(round2(dist)).build());
        }
      }
      ...
      if (detail.getTakeProfit() != null && detail.getTakeProfit() > 0) {
        ...
        if (price >= detail.getTakeProfit()) {
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("TAKE_PROFIT")
              .severity("DANGER").message(
                  "¡Precio (" + fmt(price) + " €) ha alcanzado el Take-Profit (" + fmt(
                      detail.getTakeProfit()) + " €)!").currentPrice(price)
              .limitPrice(detail.getTakeProfit()).distancePct(round2(-dist)).build());
        } else if (dist < 5) {
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("TAKE_PROFIT")
              .severity("INFO").message(
                  "Precio a solo " + fmt(dist) + "% del Take-Profit (" + fmt(detail.getTakeProfit())
      ...
      // TRAILING STOP
      if (detail.getTrailingStopPct() != null && detail.getTrailingStopPct() > 0
          && pos.getAvgPrice() != null) {
        ...
        if (distFromAvg < 0 && Math.abs(distFromAvg) >= detail.getTrailingStopPct()) {
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("TRAILING_STOP")
              .severity("WARNING").message("La caída desde P.Medio (" + fmt(Math.abs(distFromAvg))
                  + "%) supera el trailing stop (" + fmt(detail.getTrailingStopPct()) + "%)")
              .currentPrice(price).limitPrice(trailingPrice).distancePct(round2(distFromAvg))
              .build());
        }
      }
      ...
      if (detail.getDcaTargetPrice() != null && detail.getDcaTargetPrice() > 0) {
        ...
        if (price <= detail.getDcaTargetPrice()) {
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("DCA_TARGET")
              .severity("INFO").message(
                  "¡Precio (" + fmt(price) + " €) ha llegado al objetivo DCA (" + fmt(
                      detail.getDcaTargetPrice()) + " €)! Oportunidad de promediar.")
              .currentPrice(price).limitPrice(detail.getDcaTargetPrice()).distancePct(round2(dist))
              .build());
        } else if (dist < 5) {
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("DCA_TARGET")
              .severity("INFO").message(
                  "Precio a " + fmt(dist) + "% del objetivo DCA (" + fmt(detail.getDcaTargetPrice())
      ...
      if (detail.getAlertPriceAbove() != null && detail.getAlertPriceAbove() > 0) {
        if (price >= detail.getAlertPriceAbove()) {
          ...
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("ALERT_ABOVE")
              .severity("WARNING").message(
                  "Precio (" + fmt(price) + " €) ha superado la alerta superior (" + fmt(
                      detail.getAlertPriceAbove()) + " €)").currentPrice(price)
              .limitPrice(detail.getAlertPriceAbove()).distancePct(round2(dist)).build());
        }
      }
      ...
      if (detail.getAlertPriceBelow() != null && detail.getAlertPriceBelow() > 0) {
        if (price <= detail.getAlertPriceBelow()) {
          ...
          alerts.add(AlertDto.builder().ticker(ticker).name(name).color(color).type("ALERT_BELOW")
              .severity("WARNING").message(
                  "Precio (" + fmt(price) + " €) ha caído por debajo de la alerta inferior (" + fmt(
                      detail.getAlertPriceBelow()) + " €)").currentPrice(price)
              .limitPrice(detail.getAlertPriceBelow()).distancePct(round2(dist)).build());
        }
      }
    }
