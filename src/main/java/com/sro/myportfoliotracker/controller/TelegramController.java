...
/**
 * Controlador para la integración opcional con Telegram. Todos los endpoints son seguros incluso
 * cuando Telegram no está configurado.
 */
@RestController
...
public class TelegramController {
  ...
  /**
   * Devuelve el estado de la configuración de Telegram. Nunca lanza excepciones.
   */
  @GetMapping("/status")
    ...
    try {
      if (!telegramService.isEnabled()) {
        return ResponseEntity.ok(Map.of(
            "success", false,
            "message",
            "Telegram no está configurado. Usa el botón 'Configurar' para añadir tu Bot Token y Chat ID."
        ));
      }
    ...
    try {
      ...
      if (sent) {
        ...
        return ResponseEntity.ok(Map.of(
            "success", false,
            "message",
            "Configuración guardada, pero no se pudo enviar el mensaje de prueba. Revisa el token y el chat_id."
        ));
      }
    } catch (Exception e) {
      return ResponseEntity.ok(Map.of(
  ...
  public ResponseEntity<?> getChannelMessages() {
    try {
      ...
      List<Map<String, Object>> items = messages.stream().map(m -> Map.<String, Object>of(
          ...
          "date",
          m.getDateEpoch() != null ? DATE_FMT.format(Instant.ofEpochSecond(m.getDateEpoch())) : "",
      ...
      return ResponseEntity.ok(
          Map.of("items", List.of(), "channel", TelegramChannelService.CHANNEL_USERNAME));
    }
  }
  ...
  public ResponseEntity<?> pollNow() {
    try {
      ...
      return ResponseEntity.ok(
          Map.of("success", true, "messageCount", channelService.getRecentMessages().size()));
    } catch (Exception e) {
