...
public class WatchlistAlertController {
  ...
  @PostMapping
  public ResponseEntity<?> create(@PathVariable Long watchlistItemId,
      @RequestBody WatchlistAlert alert) {
    try {
  ...
  @PutMapping("/{alertId}")
  public ResponseEntity<?> update(@PathVariable Long watchlistItemId, @PathVariable Long alertId,
      @RequestBody WatchlistAlert alert) {
    try {
      alert.setWatchlistItemId(watchlistItemId);
