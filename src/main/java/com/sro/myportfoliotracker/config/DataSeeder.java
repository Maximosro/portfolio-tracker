...
public class DataSeeder {
  ...
  private void seedMarketSchedules() {
    try {
      Integer count = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM market_schedules",
          Integer.class);
      if (count != null && count > 0) {
  ...
  private void executeSqlFile(String classpathPath, String filesystemPath) {
    ...
    if (!resource.exists()) {
      log.warn("Fichero seed no encontrado ni en filesystem ({}) ni en classpath ({})",
          filesystemPath, classpathPath);
      return;
    }
